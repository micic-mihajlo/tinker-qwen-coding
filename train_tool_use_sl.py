"""
Phase 2: Supervised Learning on SWE-Bench tool-use dataset.

This script continues fine-tuning from Phase 1 checkpoint on tool-use
conversations using the Tinker API.

Usage:
    # First, run Phase 1
    python train_coding_sl.py

    # Then prepare tool-use data
...
def conversation_to_datum(
    ...
    For tool-use training, we train on:
    - Assistant messages (including tool calls)
    - We DON'T train on user messages or tool results
    ...
    try:
        ...
        for msg in messages:
            ...
            if role == "system":
                ...
                # Tool results formatted as user messages for compatibility
                parts.append(f"<|im_start|>user\n[Tool Result]\n{content}<|im_end|>")
        text = "\n".join(parts)
    ...
    # Truncate if needed (from the beginning to keep the most recent context)
    ...
    # Create weights - train on assistant responses only
    weights = [0.0] * (len(tokens) - 1)

    # Find assistant turn boundaries and set weights
    text_so_far = ""
    in_assistant = False
    ...
    # If no assistant weights found, this might be a bad example
    if sum(weights) < 10:  # At least 10 tokens to train on
        return None
...
async def train(config):
    ...
    phase1_paths_file = Path(phase1.log_path) / "paths.json"
    if not phase1_paths_file.exists():
        logger.error(f"Phase 1 paths file not found: {phase1_paths_file}")
        logger.error("Run 'python train_coding_sl.py' first to complete Phase 1.")
        sys.exit(1)

    with open(phase1_paths_file, "r") as f:
        phase1_paths = json.load(f)
    phase1_checkpoint = phase1_paths.get("final_state_path")
    logger.info(f"Resuming from Phase 1 checkpoint: {phase1_checkpoint}")

    # Check if data exists
    data_path = Path(phase2.data_path)
    if not data_path.exists():
        logger.error(f"Data file not found: {data_path}")
        logger.error("Run 'python data/prepare_swebench_tools.py' first to prepare the data.")
        sys.exit(1)

    # Create log directory
    log_path = Path(phase2.log_path)
    ...
    for conv in conversations:
        datum = conversation_to_datum(conv, tokenizer, phase2.max_length)
        if datum:
            datums.append(datum)
    ...
    n_batches = max(1, len(datums) // phase2.batch_size)
    total_steps = n_batches * phase2.num_epochs
    logger.info(f"Training for {phase2.num_epochs} epochs = {total_steps} steps")

    # Create Tinker client and load Phase 1 checkpoint
    logger.info("Loading Phase 1 checkpoint...")
    service_client = tinker.ServiceClient()

    # Check if resuming from Phase 2 checkpoint
    ...
    if checkpoint_file.exists():
        with open(checkpoint_file, "r") as f:
            ...
            if lines:
                ...
                logger.info(f"Resuming Phase 2 from step {start_step}, checkpoint: {resume_path}")
    ...
    for epoch in range(phase2.num_epochs):
        ...
        for batch_idx in range(n_batches):
            ...
            # Get batch
            batch_start = batch_idx * phase2.batch_size
            batch_end = min(batch_start + phase2.batch_size, len(datums))
            ...
            current_lr = phase2.learning_rate * lr_mult
            ...
            logger.info(
                ...
                f"Epoch {epoch+1}/{phase2.num_epochs} | "
                f"NLL: {train_nll:.4f} | "
                f"LR: {current_lr:.2e} | "
            ...
            # Save checkpoint
            if phase2.save_every > 0 and step % phase2.save_every == 0 and step > 0:
                logger.info(f"Saving checkpoint at step {step}...")
                save_result = await training_client.save_state_async(name=f"phase2-{step:06d}")
    ...
    final_save = await training_client.save_state_async(name="phase2-final")
    ...
    sampler_save = await training_client.save_weights_for_sampler_async(name="phase2-final-sampler")
    ...
    with open(paths_file, "w") as f:
        json.dump({
            "phase1_state_path": phase1_checkpoint,
            "final_state_path": final_path,
            "sampler_path": sampler_path,
        }, f, indent=2)

    logger.info(f"Paths saved to: {paths_file}")
    logger.info(f"\nTo use the trained model, use the sampler path: {sampler_path}")
